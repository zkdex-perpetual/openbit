...
library Bytes {
    ...
    function bytesToUInt256(bytes memory _bytes, uint256 _start) internal pure returns (uint160 r) {
        uint256 offset = _start + 0x20;
        require(_bytes.length >= offset, "X1");
        assembly {
            r := mload(add(_bytes, offset))
        }
    }
    ...
    // NOTE: theoretically possible overflow of (_offset + 20)
    function readUInt256(bytes memory _data, uint256 _offset) internal pure returns (uint256 newOffset, uint160 r) {
