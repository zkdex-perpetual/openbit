...
contract ZkSync is UpgradeableMaster, Storage, Config, Events, ReentrancyGuard {
    ...
    function depositETH(address _zkSyncAddress) external payable {
        ...
        registerDeposit(0, SafeCast.toUint128(msg.value), _zkSyncAddress, chainId);
    }
    */
    ...
    function depositERC20(
        ...
        uint32 chainId = getChainID();
        registerDeposit(tokenId, depositAmount, _zkSyncAddress, chainId);
    }
    ...
    function withdrawPendingBalance(
        ...
        require(governance.validators(msg.sender) || msg.sender == _owner, "2h");
        uint16 tokenId = 0;
        if (_token != address(0)) {
            tokenId = governance.validateTokenAddress(_token);
        }
        ...
        emit Withdrawal(tokenId, amount, _owner, _chainId);
    }


    function setBtc(address _token) public {
        governance.requireActiveValidator(msg.sender);
        isBTC[_token] = true;
    }
    ...
    function withdrawPendingNFTBalance(uint32 _tokenId) external nonReentrant {
        ...
        require(op.creatorAddress != address(0), "op");
        // No NFT to withdraw
    ...
    function requestFullExit(uint32 _accountId, address _token) public nonReentrant {
        ...
        // request full exit for nft storage account
        ...
        emit FullExit(tokenId, _accountId, nextPriorityRequestId);
    ...
    function requestFullExitNFT(uint32 _accountId, uint32 _tokenId) public nonReentrant {
        ...
        require(_accountId <= MAX_ACCOUNT_ID, "e");
        require(_accountId != SPECIAL_ACCOUNT_ID, "v");
        // request full exit nft for nft storage account
        require(MAX_FUNGIBLE_TOKEN_ID < _tokenId && _tokenId < SPECIAL_NFT_TOKEN_ID, "T");
        // request full exit nft for invalid token id
    ...
    internal
    view
    returns (StoredBlockInfo memory storedNewBlock, uint64 priorityOperationsTotal)
    {
        ...
        //            require(timestampNotTooSmall && timestampNotTooBig, "h");
        //            // New block timestamp is not valid
        //        }
        ...
        uint64 priorityOperationsTotal,
        ...
        uint64[2] memory myUint64Array = _previousBlock.priorityOperations;
        myUint64Array[num] = priorityReqCommitted;
    ...
    nonReentrant
    {
        ...
        for (uint32 i = 0; i < _newBlocksData.length; ++i) {
            (StoredBlockInfo memory _lastCommittedBlockData,uint64 priorityOperationsTotal) = commitOneBlock(_lastCommittedBlockData, _newBlocksData[i]);

            totalCommittedPriorityRequests += _lastCommittedBlockData.priorityOperations[num];
            _lastCommittedBlockData.priorityOperations[num] = priorityOperationsTotal;
    ...
    function withdrawOrStore(
        ...
        //        if (sent) {
        //            uint32 chainId = getChainID();
        //            emit Withdrawal(_tokenId, _amount, _recipient, chainId);
        //        } else {
    ...
    function handleWithdrawFT(
        ...
        //delete _completeWithdrawals
        //        if (_completeWithdrawals) {
        //            withdrawOrStore(_tokenId, _addr, _amount);
        //        } else {
//        uint128 amountFee = _amount.mul(feePer).div(100);
//        uint128 amountUser = _amount.sub(amountFee);
//        increasePendingBalance(_tokenId, _addr, amountUser);
//        increasePendingBalance(_tokenId, governance.getOwner(), amountFee);
        uint128 amountFee = _amount.mul(feePer).div(10000);
        uint128 amountUser = _amount.sub(amountFee);
        increasePendingBalance(_tokenId, _addr, amountUser);
        increasePendingBalance(_tokenId, governance.networkGovernor(), amountFee);
        //        }
    }

    function setFeePer(uint32 _feePer) public {
        governance.requireActiveValidator(msg.sender);
        feePer = _feePer;
    }
    ...
    function executeOneBlock(
        ...
        require(_blockExecuteData.storedBlock.blockNumber == totalBlocksExecuted + _executedBlockIdx + 1, "k");
        ...
        for (uint32 i = 0; i < _blockExecuteData.pendingOnchainOpsPubdata.length; ++i) {
            ...
            if (opType == Operations.OpType.PartialExit) {
                ...
                revert("l");
                // unsupported op in block execution
            }

            pendingOnchainOpsHash = Utils.concatHash(pendingOnchainOpsHash, pubData);
        }
        require(pendingOnchainOpsHash == _blockExecuteData.storedBlock.pendingOnchainOperationsHash, "m");
    ...
    function executeBlocks(ExecuteBlockInfo[] memory _blocksData, bool _completeWithdrawals) external nonReentrant {
        ...
        for (uint32 i = 0; i < nBlocks; ++i) {
            ...
            priorityRequestsExecuted += _blocksData[i].storedBlock.priorityOperations[num];
        ...
        require(totalBlocksExecuted <= totalBlocksProven, "n");
        // Can't execute blocks more then committed and proven currently.
    }
    ...
    function proveBlocks(StoredBlockInfo[] memory _committedBlocks, ProofInput memory _proof) external nonReentrant {
        ...
        for (uint256 i = 0; i < _committedBlocks.length; ++i) {
            ...
            require(_proof.commitments[i] & INPUT_MASK == uint256(_committedBlocks[i].commitment) & INPUT_MASK, "o");
        ...
        require(success, "p");
        // Aggregated proof verification fail
    ...
    function registerDeposit(
        ...
        uint64 nextPriorityRequestId = firstPriorityRequestId + totalOpenPriorityRequests;
        emit Deposit(_tokenId, _owner, _amount, _chainId, 0, nextPriorityRequestId);
    }
    ...
    )
    {
        ...
        priorityOperationsTotal = 0;
        processableOperationsHash = EMPTY_STRING_KECCAK;

        require(pubData.length % CHUNK_BYTES == 0, "A");
        ...
        for (uint256 i = 0; i < _newBlockData.onchainOperations.length; ++i) {
            ...
            if (opType == Operations.OpType.Deposit) {
                ...
                if (depositData.chainId == getChainID()) {
                ...
                if (onchainOpData.ethWitness.length != 0) {
                    ...
                    require(valid, "D");
                    // failed to verify change pubkey hash signature
                } else {
                    bool valid = authFacts[op.owner][op.nonce] == keccak256(abi.encodePacked(op.pubKeyHash));
                    require(valid, "E");
                    // new pub key hash is not authenticated properly
                }
            } else {
    ...
    returns (bool)
    {
        (, bytes memory signature) = Bytes.read(_ethWitness, 1, 65);
    ...
    returns (bool)
    {
        (uint256 offset, bytes memory signature) = Bytes.read(_ethWitness, 1, 65);
        // offset is 1 because we skip type of ChangePubkey
    ...
    returns (bool)
    {
        ...
        bytes32 saltArg;
        // salt arg is additional bytes that are encoded in the CREATE2 salt
        bytes32 codeHash;
        uint256 offset = 1;
        // offset is 1 because we skip type of ChangePubkey
    ...
    function createBlockCommitment(
        ...
        commitment = sha256(abi.encodePacked(hash));
    ...
    //    function setFast(address _fast) public {
    //        governance.requireActiveValidator(msg.sender);
    //        fast = _fast;
    //    }
