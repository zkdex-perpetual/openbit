...
library Operations {
    ...
    uint8 internal constant CHAIN_ID_BYTES = 4;
    ...
    uint256 internal constant PACKED_DEPOSIT_PUBDATA_BYTES =
        OP_TYPE_BYTES + ACCOUNT_ID_BYTES + TOKEN_BYTES + AMOUNT_BYTES + ADDRESS_BYTES + CHAIN_ID_BYTES;
    ...
    function readDepositPubdata(bytes memory _data) internal pure returns (Deposit memory parsed) {
        ...
        (offset, parsed.chainId) = Bytes.readUInt32(_data, offset); // chainId
...
//    function readConditionalTransferPubdata(bytes memory _data) internal pure returns (ConditionalTransfer memory parsed) {
//        // NOTE: there is no check that variable sizes are same as constants (i.e. TOKEN_BYTES), fix if possible.
//        uint256 offset = OP_TYPE_BYTES + ACCOUNT_ID_BYTES; // opType + accountId (ignored)
//        (offset, parsed.token) = Bytes.readUInt32(_data, offset); // tokenId
//        (offset, parsed.amount) = Bytes.readUInt128(_data, offset); // amount
//        offset += FEE_BYTES; // fee (ignored)
//        (offset, parsed.fact_registry_address) = Bytes.readAddress(_data, offset); // owner
//        (offset, parsed.chainId) = Bytes.readUInt32(_data, offset); // chainId
//    }
