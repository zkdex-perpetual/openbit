...
interface Events {
    ...
    event Withdrawal(uint16 indexed tokenId, uint128 amount, address user, uint32 chainId);

    event btcWithdrawal(string btcAddr, uint128 indexed amount);
    ...
    //event WithdrawalChain(address indexed token, uint128 amount, address user,uint32 chainId);
    ...
    event Deposit(uint16 indexed tokenId, address user, uint128 amount, uint32 chainId, uint256 types, uint64 serialId);

    event FullExit(uint16 indexed tokenId, uint32 indexed accountId,uint64 serialId);
