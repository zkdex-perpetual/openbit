...
contract Storage {
    ...
    mapping(address => bool) internal isBTC;
    ...
    function hashStoredBlockInfo(StoredBlockInfo memory _storedBlockInfo) internal pure returns (bytes32) {
        ...
        StoredBlockInfo1 memory convertedInfo = StoredBlockInfo1({
        blockNumber: _storedBlockInfo.blockNumber,
        pendingOnchainOperationsHash: _storedBlockInfo.pendingOnchainOperationsHash,
        timestamp: _storedBlockInfo.timestamp,
        stateHash: _storedBlockInfo.stateHash,
        commitment: _storedBlockInfo.commitment
        });

        return keccak256(abi.encode(convertedInfo));
    }
